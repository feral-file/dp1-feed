...
async function fetchFeralFileAPI(endpoint) {
  ...
  if (!response.ok) {
    throw new Error(`Feral File API request failed: ${response.status} ${response.statusText} for ${url}`);
  }
...
function buildChannelFromExhibition(exhibition, playlistUrls) {
  ...
  // Build summary
  let summary = exhibition.note || exhibition.noteBrief || 
...
async function uploadPlaylist(feedEndpoint, apiKey, playlist) {
  ...
  if (!response.ok) {
    ...
    throw new Error(`Failed to upload playlist: ${response.status} ${response.statusText}\n${errorText}`);
  }
...
async function processExhibition(feedEndpoint, apiKey, exhibitionPath) {
  ...
  const files = fs.readdirSync(exhibitionPath)
...
async function main() {
  ...
  if (!apiKey || !feedEndpoint || !playlistsPath) {
    console.error('Usage: node upload-to-feed.js --api-key <key> --feed-endpoint <url> --playlists-path <path> [--dry-run] [--output <summary-file>]');
    console.error('\nRequired flags:');
    ...
    console.error('\nExamples:');
    console.error('  node scripts/upload-to-feed.js --api-key YOUR_API_KEY --feed-endpoint https://feed.feralfile.com --playlists-path ./playlists');
    console.error('  node scripts/upload-to-feed.js --api-key YOUR_API_KEY --feed-endpoint http://localhost:8787 --playlists-path ./playlists/net-evil-das');
    console.error('  node scripts/upload-to-feed.js --api-key YOUR_API_KEY --feed-endpoint https://feed.feralfile.com --playlists-path ./playlists --dry-run');
    console.error('  node scripts/upload-to-feed.js --api-key YOUR_API_KEY --feed-endpoint https://feed.feralfile.com --playlists-path ./playlists --output SUMMARY.md');
    console.error('  node scripts/upload-to-feed.js --api-key YOUR_API_KEY --feed-endpoint https://feed.feralfile.com --playlists-path ./playlists --dry-run --output DRY-RUN.md');
    process.exit(1);
  }
  ...
  try {
    if (stat.isDirectory()) {
      ...
      if (hasJsonFiles) {
        ...
        console.log(`  Total Playlists: ${results.reduce((sum, r) => sum + (r.playlists?.length || 0), 0)}`);
        console.log('='.repeat(80));
